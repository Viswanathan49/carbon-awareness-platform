...
export default function CarbonAnalytics({ userTotal = 4.5 }) {
  ...
  return (
    <section className="w-full max-w-5xl mx-auto mt-12 mb-24 px-4 block clear-both relative z-10">
        ...
        <div className="p-6 bg-gray-50 dark:bg-black/40 border border-gray-200 dark:border-white/5 rounded-2xl">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            ...
            <select 
              ...
              className="bg-white dark:bg-[#1a1a1a] text-gray-900 dark:text-[#0FDE72] border border-gray-300 dark:border-white/20 rounded-lg px-4 py-2 text-sm focus:outline-none"
            >
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="w-full bg-gray-200 dark:bg-gray-800 rounded-full h-3 overflow-hidden">
                <div className="bg-[#0FDE72] h-full rounded-full" style={{ width: `${calculatePercentage(userTotal, baselines[region])}%` }}></div>
              </div>
            </div>
          </div>
        </div>

        {/* Charts - Fixed heights on wrapper divs with aspect control to prevent -1 height warnings */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="w-full bg-gray-50 dark:bg-black/40 border border-gray-200 dark:border-white/5 rounded-2xl p-5 flex flex-col min-h-[320px]">
            ...
            <div className="w-full h-[250px] relative">
              <ResponsiveContainer width="100%" height="100%" minHeight={250}>
                <LineChart data={historyData}>
                  ...
                  <Tooltip contentStyle={{ backgroundColor: '#fff', border: 'none', borderRadius: '8px', color: '#000' }} />
                  <Line type="monotone" dataKey="emissions" stroke="#00D1FF" strokeWidth={3} dot={{ fill: '#fff', stroke: '#00D1FF', strokeWidth: 2 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
